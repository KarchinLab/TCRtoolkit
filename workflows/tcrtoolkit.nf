...
workflow TCRTOOLKIT {
    ...
    // template_discovery_brief.qmd's VDJdb section falls back to reading raw
    // (unconverted) input files when convert_dir is empty, which only has AIRR-
    // standard frequency columns for input_format 'airr' - for 'adaptive'/
    // 'cellranger' it needs the AIRR-converted files, so those are only staged
    // when CONVERT actually ran (mirrors the same input_format branch above).
